...
/** A value an operator may have saved for a declared setting. */
...
export interface ExtensionSettings {
   ...
   *
   * Optional because `sdk.settings` is attached for two independent reasons —
   ...
   * it is reading.
   */
  tautulli?: Readonly<SeerrTautulliSettings>;
 ...
 * two. The host's manifest key pattern rejects it, so no extension can declare a
 * permission that collides with the sentinel.
 */
