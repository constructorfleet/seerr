...
mediaRoutes.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      // The removal itself lives in `@server/lib/mediaRemoval` because
      // `sdk.media.remove` performs exactly the same operation for extensions.
      // Two copies would be two places for a Radarr-versus-Sonarr or 4K-variant
      // bug to be fixed only once.
      await removeMediaFromServarr(media, is4k);

      // The helper mutates but does not save, so that a caller can fold the
      // removal into one write with whatever bookkeeping it does alongside.
      await mediaRepository.save(media);
      ...
      // A missing Radarr/Sonarr server is the operator's configuration, not a
      // server fault, so it keeps its own 409 rather than falling through to the
      // 500 below.
